...
/**
 * Execute the ceil distributed community detection.
...
object Main {
  ...
  def main(args: Array[String]) {
   ...
   rootLogger.setLevel(Level.ERROR)
    ...
    val runner = new CeilOutput(minProgress,progressCounter,outputdir)
    val t1=System.nanoTime()
    runner.run(sc, graph)
    val t2=System.nanoTime()
    println("Time taken = "+ (t2-t1)/Math.pow(10, 9) + " seconds" )
